...
<body>
    ...
    <x-app-shell :user="$user">
        ...
        <main class="app-page-content space-y-6">
            ...
            <x-page-header :title="'Bună, ' . $user->first_name" :description="$company
                ? 'Iată situația firmei ' . $company->name . ':'
                : 'Nu ai nicio firmă asociată contului.'">
                <x-slot:actions>
                                ...
                                <li class="flex items-center justify-between px-5 py-3">
                                    <div class="min-w-0">
                                        <p class="truncate font-medium text-slate-900 dark:text-slate-100">
                                            {{ $payment->invoice?->client?->full_name ?? '—' }}</p>
                                        <p class="mt-0.5 text-xs text-slate-400">
                                            {{ $payment->payment_date?->translatedFormat('d M') }}</p>
                                    </div>
