...
<body>
    ...
    <x-app-shell>
        ...
        <main class="app-page-content space-y-6">
            ...
            <x-page-header title="Jurnal de audit" :description="'Cine și ce a modificat în firma ' . $company->name" />
            ...
            <form method="GET" action="{{ route('audit-log.index') }}"
                ...
                <label class="text-sm">
                    ...
                    <select name="event" class="form-input">
                        ...
                        @foreach (\App\Models\Audit::EVENT_LABELS as $value => $label)
                            <option value="{{ $value }}" @selected(($filters['event'] ?? null) === $value)>{{ $label }}
                            </option>
                        @endforeach
                    </select>
                </label>
                ...
                <div class="ui-button-group">
                    ...
                    <a href="{{ route('audit-log.index') }}" class="ui-btn ui-btn-secondary">
                        Resetează
                    </a>
                </div>
            </form>
                            ...
                            @forelse ($audits as $audit)
                                <tr class="align-top">
                                    ...
                                    <td class="px-5 py-4 whitespace-nowrap">
                                        ...
                                        @endphp
                                        <span
                                            class="inline-block px-2 py-1 rounded-md text-xs font-medium {{ $tone }}">
                                            {{ $audit->eventLabel() }}
                                        </span>
                                    </td>
                                    <td class="px-5 py-4 whitespace-nowrap">
                                        ...
                                        @else
                                            <ul class="space-y-1">
                                                @foreach ($modified as $field => $change)
                                                    <li class="text-xs">
                                                        <span
                                                            class="text-slate-500 dark:text-slate-400">{{ \App\Models\Audit::fieldLabel($field) }}:</span>
                                                        @if (array_key_exists('old', $change))
                                                            <span
                                                                class="line-through text-slate-400">{{ $show($change['old']) }}</span>
                                                        @endif
                                                        @if (array_key_exists('new', $change))
                                                            <span
                                                                class="font-medium text-slate-900 dark:text-slate-100">{{ $show($change['new']) }}</span>
                                                        @endif
                                                    </li>
                                                @endforeach
                                            </ul>
