                    ...
                    @else
                        ...
                        @if ($companies->count() > 1)
                            <div class="ui-card px-5 py-4">
                                <form action="{{ route('administrator.settings.company') }}" method="GET"
                                    class="flex flex-col sm:flex-row sm:items-end gap-3">
                                    <div class="flex-1">
                                        <label for="firma"
                                            class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Firma
                                            pe care o editezi</label>
                                        <select id="firma" name="firma" class="form-input">
                                            @foreach ($companies as $c)
                                                <option value="{{ $c->id }}" @selected($c->id === $company->id)>
                                                    {{ $c->name }} · CUI {{ $c->cui }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                        ...
                        <div class="ui-card overflow-hidden">
                            ...
                            <form action="{{ route('administrator.companies.update', $company) }}" method="POST"
                                class="px-5 py-5 space-y-5" data-partial>
                                @csrf
                                @method('PUT')
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                    <div class="sm:col-span-2">
                                        <label for="name"
                                            class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Denumire</label>
                                        <input type="text" id="name" name="name" required maxlength="255"
                                            value="{{ old('name', $company->name) }}" class="form-input">
                                        @error('name')
                                            <p class="text-xs text-rose-600 mt-1">{{ $message }}</p>
                                        @enderror
                                    </div>
                                    <div>
                                        <label for="juridical_form"
                                            class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Formă
                                            juridică</label>
                                        <select id="juridical_form" name="juridical_form" required class="form-input">
                                            @foreach ($formeJuridice as $forma)
                                                <option value="{{ $forma }}" @selected(old('juridical_form', $company->juridical_form) === $forma)>
                                                    {{ $forma }}</option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div>
                                        <label for="cui"
                                            class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">CUI</label>
                                        <input type="text" id="cui" name="cui" required maxlength="20"
                                            value="{{ old('cui', $company->cui) }}" class="form-input">
                                        @error('cui')
                                            <p class="text-xs text-rose-600 mt-1">{{ $message }}</p>
                                        @enderror
                                    </div>
                                    <div>
                                        <label for="trade_registry_number"
                                            class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Nr.
                                            Registrul Comerțului</label>
                                        <input type="text" id="trade_registry_number" name="trade_registry_number"
                                ...
                                <div>
                                    <label for="social_capital"
                                        class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Capital
                                        social (RON)</label>
                                    <input type="number" id="social_capital" name="social_capital" required
                                        step="0.01" min="0"
                        ...
                        <div class="ui-card overflow-hidden">
                            ...
                            <form action="{{ route('administrator.companies.update', $company) }}" method="POST"
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div>
                                        <label for="county"
                                            class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Județ</label>
                                        <input type="text" id="county" name="county" required maxlength="255"
                                            value="{{ old('county', $company->county) }}" class="form-input">
                                        @error('county')
                                            <p class="text-xs text-rose-600 mt-1">{{ $message }}</p>
                                        @enderror
                                    </div>
                                    <div>
                                        <label for="city"
                                            class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Localitate</label>
                                        <input type="text" id="city" name="city" required maxlength="255"
                                            value="{{ old('city', $company->city) }}" class="form-input">
                                        @error('city')
                                            <p class="text-xs text-rose-600 mt-1">{{ $message }}</p>
                                        @enderror
                                    </div>
                                ...
                                <div>
                                    <label for="address"
                                        class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Adresă</label>
                                    <input type="text" id="address" name="address" required maxlength="255"
                                        value="{{ old('address', $company->address) }}" class="form-input">
                                    @error('address')
                                        <p class="text-xs text-rose-600 mt-1">{{ $message }}</p>
                                    @enderror
                                </div>
                        ...
                        <div class="ui-card overflow-hidden">
                            ...
                            <form action="{{ route('administrator.companies.update', $company) }}" method="POST"
                                class="px-5 py-5 space-y-4" data-partial>
                                @csrf
                                @method('PUT')
                                ...
                                <label class="flex items-start gap-3">
                                    <input type="checkbox" name="vat_payer" value="1"
                                        @checked(old('vat_payer', $company->vat_payer))
                                        class="mt-0.5 w-4 h-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500">
                                    <span>
                                        <span
                                            class="block text-sm font-medium text-slate-900 dark:text-slate-100">Firmă
                                            înregistrată în scopuri de TVA</span>
                                        <span class="block text-xs text-slate-500 dark:text-slate-400 mt-0.5">Bifează
                                            doar după ce ai primit codul de TVA de la ANAF.</span>
                                    </span>
                                </label>
                                ...
                                <div class="flex items-start gap-2 px-3 py-2 rounded-lg bg-amber-50 text-amber-800">
                                    ...
                                    <p class="text-xs">Modificarea se aplică doar facturilor emise de acum înainte.
                                        Facturile deja emise își păstrează regimul de TVA de la data emiterii.</p>
                                </div>
