...
<body>
    ...
    <x-app-shell>
        ...
        <header class="app-page-toolbar">
            <x-company-switcher :companies="$companies" :active-company="$company"> <x-slot:meta>
                    ...
                    @else
                        ...
                        @foreach ($documentTypes as $type)
                            ...
                            <div class="ui-card overflow-hidden">
                                ...
                                @forelse ($list as $s)
                                    <div
                                        class="px-5 py-4 border-b border-slate-100 flex flex-col xl:flex-row xl:items-end gap-4 {{ $s->is_active ? '' : 'bg-slate-50' }}">
                                        ...
                                        <form action="{{ route('administrator.series.update', $s) }}" method="POST"
                                            ...
                                            <div>
                                                <label for="prefix-{{ $s->id }}"
                                                    class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Prefix</label>
                                                <input id="prefix-{{ $s->id }}" type="text" name="prefix"
                                                    maxlength="10" required value="{{ $s->prefix }}"
                                                    @disabled($s->is_used) class="form-input font-mono uppercase">
                                            </div>

                                            <div>
                                                <label for="start-{{ $s->id }}"
                                                    class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Nr.
                                                    pornire</label>
                                                <input id="start-{{ $s->id }}" type="number"
                                                    name="start_number" min="1" required
                                                    value="{{ $s->start_number }}" @disabled($s->is_used)
                                                    class="form-input">
                                            </div>

                                            <div>
                                                <span
                                                    class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Următorul
                                                    document</span>
                                                <p
                                                    class="px-3 py-2 text-sm font-mono text-slate-700 dark:text-slate-300">
                                            ...
                                            <div>
                                                @if ($s->is_used)
                                                    <p class="text-xs text-slate-400 leading-tight">
                                                        Serie folosită — prefixul și numărul de pornire nu se mai pot
                                                        modifica.
                                                    </p>
                                                @else
                                        ...
                                        <div class="flex flex-wrap items-center gap-2 shrink-0">
                                            ...
                                            @if ($s->is_default)
                                                <span
                                                    class="text-xs px-2 py-1 rounded-full bg-indigo-50 text-indigo-700 font-medium">Implicită</span>
                                            @endif

                                            @if (!$s->is_active)
                                                <span
                                                    class="text-xs px-2 py-1 rounded-full bg-slate-200 text-slate-600 dark:text-slate-300 font-medium">Inactivă</span>
                                            @endif

                                            @if (!$s->is_default && $s->is_active)
                                                <form action="{{ route('administrator.series.default', $s) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('PATCH')
                                                    <button type="submit" class="ui-btn ui-btn-secondary">
                                            ...
                                            @unless ($s->is_default && $s->is_active)
                                                <form action="{{ route('administrator.series.status', $s) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('PATCH')
                                                    <button type="submit"
                                ...
                                <form action="{{ route('administrator.series.store') }}" method="POST"
                                    ...
                                    <div>
                                        <label for="prefix-{{ $type->value }}"
                                            class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Prefix
                                            serie nouă</label>
                                        <input type="text" id="prefix-{{ $type->value }}" name="prefix"
                                            maxlength="10" required placeholder="{{ $type->defaultPrefix() }}"
                                            class="form-input font-mono uppercase">
                                    </div>

                                    <div>
                                        <label for="start-{{ $type->value }}"
                                            class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Nr.
                                            pornire</label>
                                        <input type="number" id="start-{{ $type->value }}" name="start_number"
                                            min="1" required value="1" class="form-input">
                                    </div>

                                    <div class="flex items-center gap-2 pb-2">
                                        <input type="hidden" name="is_default" value="0">
                                        <input type="checkbox" id="default-{{ $type->value }}" name="is_default"
                                            value="1"
                                            class="w-4 h-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500">
                                        <label for="default-{{ $type->value }}"
                                            class="text-sm text-slate-600 dark:text-slate-300">Serie implicită</label>
                                    </div>
